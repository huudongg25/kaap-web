...
function MyLibsVideoHome(props: any) {
  ...
  const [, setOptionState] = useState(true)
  ...
  const dataGetProfile = {
    avatar: localStorage.getItem('avatar-img'),
    name: localStorage.getItem('user-name')
  }

  const [dataProfile] = useState<any>(dataGetProfile)
  ...
  const fnAddToFavorite = async () => {
    try {
      ...
      if (idApiFolder === '') {
        temp = {
          favorite: true,
          folderId: dataItemChoose.folder.id
        }
  ...
  useEffect(() => {
    ...
    setListShow(props.showIconVideo)
  ...
  useEffect(() => {
    setChangeSelectVideo(
      dataState &&
        dataState.map((item: any) => {
          return item.roleGeneral
        })
    )
  }, [props])
  ...
  function handleRouterLink(item: any) {
    ...
    const tempUrl = (window.location.pathname + 'watch-video').split('/').slice(0, 3).join('/') + '/'
    ...
    <div>
      {/*{showSpinner && <Spinner/>}*/}
      {
        <div>
          ...
          {listShow && (
            <TableContainer sx={{ marginTop: 2, width: '100%' }} component={Paper}>
              <Table sx={{ minWidth: 650 }} size='small' aria-label='a dense table'>
                <TableBody>
                  ...
                  {dataState
                    ...
                    .map((row: any, indexTable: number) => (
                      <TableRow
                        onClick={() => {
                          router.push(`my-library/watch-video/${row.id}`)
                        }}
                        key={indexTable}
          ...
          >
            <Box sx={{ ...styleModalShare, width: 550 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <FormControl sx={{ width: '100px', padding: '5px', height: '40px' }}>
                  <Select
                    sx={{
                      width: '100px',
                      height: '40px',
...
export default memo(MyLibsVideoHome)
