...
import { deleteFolderId, postListFolderIdVer, postListFolderId } from '../../service/myLibrary'
...
function MyLibsFolderHome(props: any) {
  const dispatch = useDispatch()
  const router = useRouter()
  const [dataState, setDataState] = useState(props.props)
  const [optionState, setOptionState] = useState()
  const [listShow, setListShow] = useState(props.showIconList)
  ...
  const [valueAutoCompleteShare, setValueAutoCompleteShare] = useState<any>(null)
  ...
  const handleRouterLink = (item: any) => {
    ...
    const tempUrl = (window.location.pathname + 'folder').split('/').slice(0, 3).join('/') + '/'
  ...
  const fnRenameFolderIdApi = async (id: string, name: string, favorite: boolean, parentId: string, roleGeneral: 1) => {
    try {
  ...
  useEffect(() => {
    setValueAutoCompleteShare(valueAutoCompleteShare)
  }, [valueAutoCompleteShare])
  ...
  return (
    <div>
      {!listShow && (
        <Grid className={'grid-folder'} sx={{ marginTop: '-1.5rem' }} container spacing={4}>
          {dataState &&
                          ...
                          <Box sx={{ flexGrow: 0 }}>
                            ...
                            >
                              {listSelect.map((option: any, index: number) => (
                                <MenuItem sx={{ paddingRight: '10px' }} key={index} onClick={handleCloseUserMenu}>
...
export default MyLibsFolderHome
